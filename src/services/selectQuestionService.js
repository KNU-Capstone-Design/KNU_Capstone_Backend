...
export async function selectQuestion(email) {
    ...
    // 발송할 새로운 질문 조회 최적화
    const nextQuestion = await findNextQuestion(selectedCategory, sentQuestionIds, user._id);
 ...
 */
async function findNextQuestion(category, sentQuestionIds, userId) {
    ...
    if (sentQuestionIds.length > 500) {
        ...
        nextQuestion = randomQuestions.length > 0 ? randomQuestions[0] : null;
    } else {
        // 일반적인 경우 표준 쿼리 사용
    ...
    // 카테고리 내 질문을 모두 발송한 경우, 가장 오래 전에 보낸 질문부터 다시 순환 발송
    return nextQuestion ?? await findOldestSentQuestion(userId, category);
}

/**
 * 카테고리 내 질문을 모두 소진했을 때 순환 발송할 질문 조회
 * (가장 오래 전에 발송된 질문을 다시 선택)
 */
async function findOldestSentQuestion(userId, category) {
    const oldestActivity = await UserActivity.findOne({ user: userId, category })
        .sort({ createdAt: 1 })
        .select('question')
        .lean();

    if (!oldestActivity) return null; // 해당 카테고리에 질문 자체가 없는 경우

    return Question.findById(oldestActivity.question).lean();
}
